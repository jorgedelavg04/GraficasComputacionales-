...
function createSquare(gl) 
{
    ...
    var square = {buffer:vertexBuffer, vertSize:3, nVerts:6, primtype:gl.TRIANGLE_STRIP};
    return square;
}
...
function createRhombus(gl)
{
    ...
    // The resulting object contains the vertexbuffer, the size of the vertex structure (3 floats, x, y, z), the number of vertices to be drawn, the the primitive to draw.
    var rhombus = {buffer:vertexBuffer, vertSize:3, nVerts:6, primtype:gl.TRIANGLES};
    return rhombus;
}  
...
function createSphere(gl, radius)
{
  ...
  var triangleAmount = 360;
  var verts = [0.0,0.0,0.0];
  var attributos = 3;  
  var doublePI = 2*Math.PI;

  for (var i = triangleAmount/10; i <= triangleAmount*0.85; i++) {
        
        verts.push(radius*Math.cos(i*doublePI/triangleAmount));
        verts.push(radius*Math.sin(i*doublePI/triangleAmount)); 
        verts.push(0.0);
  }

  gl.bufferData(gl.ARRAY_BUFFER, new Float32Array(verts), gl.STATIC_DRAW);

  var sphere = {buffer:vertexBuffer, vertSize:3, nVerts:verts.length/attributos, primtype:gl.TRIANGLE_FAN};
  return sphere;
